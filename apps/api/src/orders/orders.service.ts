  ...
  private async issueInvoiceIfNeeded(orderId: string): Promise<void> {
    try {
      ...
      if (res.status === "issued" && res.invoiceId) {
        ...
        // 2026-09-11: taslağı resmileştir (e-Arşiv/e-Fatura) + PDF + müşteri maili. Fire-and-forget:
        // Paraşüt işi 10-40 sn sürebilir, durum güncelleme yanıtı beklemez; hata → saatlik cron.
        void this.invoice?.finalize(orderId);
