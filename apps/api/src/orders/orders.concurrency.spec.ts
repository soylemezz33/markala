...
describe("OrdersService.create — cari kredi limiti ATOMİK kontrol (transaction içinde)", () => {
  it("limit dahilinde cari sipariş: bakiye kontrolü tx içinde geçer, debit yazılır", async () => {
    ...
    const svc = new OrdersService(prisma as never, makeParasut() as never, makeSettings() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true), sendNewOrderAdminEmail: vi.fn().mockResolvedValue(true), sendOrderShippedEmail: vi.fn().mockResolvedValue(true), sendOrderDeliveredEmail: vi.fn().mockResolvedValue(true) } as never, { isEnabled: () => false } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never);
