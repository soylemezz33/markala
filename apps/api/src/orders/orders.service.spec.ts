...
/** ParasutService mock — varsayılan no-op (yapılandırılmamış davranışı). */
function makeParasut(overrides: Record<string, unknown> = {}) {
  return {
    createInvoiceFromOrder: vi.fn().mockResolvedValue({ invoiceId: "", status: "skipped" }),
    ...overrides,
  };
}
...
describe("OrdersService.create — validasyon", () => {
  it("boş items listesi → BadRequestException", async () => {
    ...
    const svc = new OrdersService(prisma as never, makeParasut() as never, makeSettings() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true), sendOrderShippedEmail: vi.fn().mockResolvedValue(true), sendOrderDeliveredEmail: vi.fn().mockResolvedValue(true) } as never, { isEnabled: () => false } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never);
...
describe("OrdersService.create — indirim subtotal clamp (#1)", () => {
  const COUPON_BASE = {
    id: "cp1", code: "BIG80", isActive: true, validFrom: null, validUntil: null,
