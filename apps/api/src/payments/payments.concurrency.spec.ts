...
describe("PaymentsService.handlePaydownCallback — cari tahsilat idempotency + fazla-alacak", () => {
  ...
  it("başarılı tahsilat: claim kazanır → tek credit kaydı oluşur (atomik, tx içinde)", async () => {
    ...
    const svc = new PaymentsService(prisma as never, makeIyzico(SUCCESS()) as never, makeConfig() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true) } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never, { earnForOrder: () => Promise.resolve() } as never);
  ...
  it("fazla-alacak engellenir: init↔callback arasında borç düştüyse tahsilat bakiyeyle kırpılır", async () => {
    ...
    const svc = new PaymentsService(prisma as never, makeIyzico(SUCCESS("pay1", 100)) as never, makeConfig() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true) } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never, { earnForOrder: () => Promise.resolve() } as never);
  ...
  it("tutar uyuşmazlığı (price≠amount) → hata, claim/credit hiç çalışmaz", async () => {
    ...
    const svc = new PaymentsService(prisma as never, makeIyzico(SUCCESS("pay1", 999)) as never, makeConfig() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true) } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never, { earnForOrder: () => Promise.resolve() } as never);
...
describe("PaymentsService.handleCallback — sipariş ödemesi idempotency (koşullu updateMany)", () => {
  ...
  it("başarı: koşullu updateMany (paymentStatus≠basarili) ile işaretler → idempotent", async () => {
    ...
    const svc = new PaymentsService(prisma as never, makeIyzico(OK) as never, makeConfig() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true) } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never, { earnForOrder: () => Promise.resolve() } as never);
  ...
  it("başarısız ödeme: koşullu updateMany (paymentStatus=beklemede) ile işaretler", async () => {
    ...
    const svc = new PaymentsService(prisma as never, makeIyzico(failResult) as never, makeConfig() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true) } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never, { earnForOrder: () => Promise.resolve() } as never);
...
describe("PaymentsService.reconcilePendingPayments — kurtarma yarışı (koşullu updateMany)", () => {
  it("yalnız hâlâ 'beklemede' siparişi kurtarır; callback önce işlediyse (count=0) no-op", async () => {
    ...
    const svc = new PaymentsService(prisma as never, iyzico as never, makeConfig() as never, { sendOrderConfirmationEmail: vi.fn().mockResolvedValue(true) } as never, { sendPurchase: vi.fn().mockResolvedValue(undefined) } as never, { earnForOrder: () => Promise.resolve() } as never);
