...
function makePrisma() {
  return {
    ...
    productOption: {
      groupBy: vi.fn().mockResolvedValue([]), // fiyatlı grup yok → ucuz MIN(price) yolu
    },
    product: {
...
describe("ProductsService.findAll displayPrice", () => {
  it("findAll displayPrice: tek fiyatlı grup → MIN(price) yolu; satır yoksa null", async () => {
    const prisma = makePrisma();
    prisma.product.findMany.mockResolvedValue([{ id: "a" }, { id: "b" }] as any);
    prisma.productOption.groupBy.mockResolvedValue([{ productId: "a", groupKey: "g" }] as any);
    prisma.productPrice.groupBy.mockResolvedValue([{ productId: "a", _min: { price: "30" } }] as any);
    const svc = new ProductsService(prisma as never, makeSettings() as never);
