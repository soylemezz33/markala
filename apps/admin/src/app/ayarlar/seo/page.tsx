...
export default function SeoSettingsPage() {
  const [defaultTitle, setDefaultTitle] = useState("Markala — Matbaa ve Reklam Ürünleri");
  const [titleTemplate, setTitleTemplate] = useState("%s · Markala");
  ...
  const [keywords, setKeywords] = useState("matbaa, reklam ürünleri, kartvizit baskı, broşür baskı, online matbaa, 324 ajans, markala");
  const [ogImage, setOgImage] = useState("/api/mockup?theme=brand&w=1200&h=630");
  const [gscToken, setGscToken] = useState("");
  const [ga4Id, setGa4Id] = useState("");
  const [robotsAllowAi, setRobotsAllowAi] = useState(false);
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <Card title="Genel Meta" icon={Globe}>
          <Field label="Varsayılan Site Başlığı (anasayfa)">
            <input className={cls} value={defaultTitle} onChange={(e) => setDefaultTitle(e.target.value)} />
            <span className="text-[11px] text-ink-500 mt-1 block">{defaultTitle.length}/60 karakter</span>
          </Field>
          <Field label="Başlık Şablonu (alt sayfalar için)">
            <input className={cls + " font-mono"} value={titleTemplate} onChange={(e) => setTitleTemplate(e.target.value)} />
            <span className="text-[11px] text-ink-500 mt-1 block">%s = sayfa başlığı yer tutucusu</span>
          </Field>
          <Field label="Varsayılan Meta Açıklama">
            <textarea rows={4} className={cls} value={defaultDescription} onChange={(e) => setDefaultDescription(e.target.value)} />
            <span className="text-[11px] text-ink-500 mt-1 block">{defaultDescription.length}/160 karakter</span>
          </Field>
          <Field label="Varsayılan Anahtar Kelimeler (virgülle)">
            <textarea rows={2} className={cls + " font-mono text-xs"} value={keywords} onChange={(e) => setKeywords(e.target.value)} />
          </Field>
        </Card>

        <Card title="Open Graph & Sosyal Önizleme" icon={ChartLine}>
          <Field label="Open Graph Görseli (1200×630)">
            <input className={cls} value={ogImage} onChange={(e) => setOgImage(e.target.value)} />
          </Field>
        ...
        <Card title="Search Console & Analytics" icon={ShieldCheck}>
          <Field label="Google Search Console Doğrulama Kodu">
            <input className={cls + " font-mono"} value={gscToken} onChange={(e) => setGscToken(e.target.value)} placeholder="google-site-verification=..." />
          </Field>
          <Field label="Google Analytics 4 Measurement ID">
            <input className={cls + " font-mono"} value={ga4Id} onChange={(e) => setGa4Id(e.target.value)} placeholder="G-XXXXXXXXXX" />
          </Field>
          <Field label="Microsoft Clarity Project ID (opsiyonel)">
            <input className={cls + " font-mono"} placeholder="abc123xyz" />
          </Field>
        </Card>
...
function Toggle({
  ...
  return (
    <label className="flex items-start justify-between gap-3 py-2 cursor-pointer">
      ...
      >
        <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-paper-50 shadow transition-transform ${checked ? "translate-x-5" : ""}`} />
      </button>
    </label>
  );
}
