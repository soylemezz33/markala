...
/**
 * Doğrudan Drive yüklemesi — 1. adım: oturum aç (2026-09-03, 1000 MB).
 * Tarayıcı → bu route → POST /api/orders/:id/items/:itemId/tasarim/drive-oturum (ORDERS_DESIGN).
 * Dönen `uploadUrl`'e tarayıcı dosyayı PARÇA PARÇA kendisi PUT eder (sunucuya uğramaz).
 */
export async function POST(req: NextRequest, { params }: { params: { orderId: string; itemId: string } }) {
  ...
  let res: Response;
  try {
    res = await fetch(`${API_URL}/api/orders/${params.orderId}/items/${params.itemId}/tasarim/drive-oturum`, {
      method: "POST",
