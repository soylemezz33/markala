...
export default function ProductEditPage({ params }: Props) {
  ...
  const [startingPrice, setStartingPrice] = useState(product.startingPrice ?? product.basePrice);
  ...
  const [keywords, setKeywords] = useState((product.seo?.keywords ?? []).join(", "));

  // Matrix fiyat editor için
    ...
    <AdminShell>
      <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-semibold text-ink-900">{product.name}</h1>
            <p className="text-xs text-ink-500 font-mono">{product.slug} · {product.sku ?? "SKU yok"}</p>
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          <Card title="Temel Bilgiler">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              ...
              <Field label="Üretim Süresi">
                <input value={productionTime} onChange={(e) => setProductionTime(e.target.value)} className={inputCls} />
              </Field>
          ...
          {matrixParam && (
            <Card title="Fiyat Matrisi (Paket × Adet)">
              <p className="text-xs text-ink-500 mb-3">
                Tabloda her hücre düzenlenebilir. Yapılan değişiklikler "Kaydet"e basınca uygulanır.
                Boş bırakırsanız o kombinasyon satışa çıkmaz (— işareti).
              </p>
              <div className="overflow-x-auto border border-paper-200 rounded-md">
                <table className="w-full text-xs">
                  <thead className="bg-paper-100/60 text-ink-500">
                    <tr>
                      <th className="text-left px-3 py-2 font-semibold sticky left-0 bg-paper-100/60 z-10">Paket / Ebat</th>
                      {matrixParam.cols?.map((c) => (
                        <th key={c.id} className="text-center px-2 py-2 font-semibold whitespace-nowrap">
                          {c.label}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-paper-200">
                    {matrixParam.rows?.map((r) => (
                      <tr key={r.id} className="hover:bg-paper-100/40">
                        <th scope="row" className="text-left px-3 py-2 font-medium text-ink-900 sticky left-0 bg-paper-50">
                          <div className="flex flex-col">
                            <span className="text-xs">
                              {r.group && <span className="text-[9px] text-brand-700 font-bold mr-1">[{r.group}]</span>}
                              {r.label}
                            </span>
                            {r.sublabel && <span className="text-[10px] text-ink-500 font-normal mt-0.5 max-w-[300px] truncate">{r.sublabel}</span>}
                          </div>
                        </th>
                        {matrixParam.cols?.map((c) => {
                          const cell = matrixParam.cells?.find((x) => x.rowId === r.id && x.colId === c.id);
                          return (
          ...
          <Card title="SEO Ayarları">
            ...
            <Field label="Anahtar Kelimeler (virgülle ayrılmış)">
              <input value={keywords} onChange={(e) => setKeywords(e.target.value)} className={inputCls + " font-mono text-xs"} />
            </Field>
          </Card>
        </div>
        ...
        <div className="space-y-5">
          <Card title="Yayın Durumu">
            ...
            <label className="flex items-center justify-between gap-3 cursor-pointer mt-3">
              <span className="text-sm font-medium text-ink-900">Anasayfada öne çıkar</span>
          ...
          <Card title="Görseller">
            <div className="grid grid-cols-3 gap-2">
              {product.images.slice(0, 6).map((img, i) => (
                ...
                <img
                  key={i}
                  src={img.startsWith("http") || img.startsWith("/api") ? img : `https://markala.com.tr${img}`}
          ...
          <Card title="Tehlikeli Bölge">
            <button
              onClick={() => confirm(`"${product.name}" ürünü silinecek. Emin misiniz?`)}
