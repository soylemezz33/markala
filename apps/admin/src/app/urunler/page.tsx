  ...
  return (
    <AdminShell>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-paper-200">
              {filtered.map((p) => {
                ...
                const startingPrice = p.startingPrice ?? p.basePrice;
                return (
                  <tr key={p.slug} className="hover:bg-paper-100/40">
                    ...
                    <td className="px-4 py-3 text-ink-700 hidden lg:table-cell">{cat?.name ?? "—"}</td>
      ...
      <p className="mt-4 text-xs text-ink-500">
        ⓘ Backend bağlandığında bu liste <code className="font-mono px-1 py-0.5 rounded bg-paper-100">GET /api/products</code> üzerinden gerçek veriyle gelecek.
        Şu an mock-data paketi okunuyor.
      </p>
    </AdminShell>
  );
}
