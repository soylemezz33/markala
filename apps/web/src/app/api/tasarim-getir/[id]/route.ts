...
// Kayıtlı tasarımı backend'den getir (yükle). Üye → Authorization; misafir → ?sessionId.
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const url = `${API_BASE}/api/designs/${encodeURIComponent(params.id)}${
  ...
  try {
    ...
    console.error("[tasarim-getir] backend erişilemedi:", (err as Error).message);
