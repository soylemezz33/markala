 ...
 * - HER fonksiyon API hatası/boş yanıtta MOCK'a düşer → blog ASLA kırılmaz.
...
// ─────────────────────────────────────────────────────────────────────────────
// MOCK içerik (API hatası/boş → fallback). Eskiden ana kaynaktı; artık güvenlik ağı.
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * coverImage varsa görsel /api/mockup yerine doğrudan kullanılır; yoksa slug ile mock theme'e düşülür.
 */
function mapPost(p: Record<string, unknown>): BlogPost {
  const slug = String(p.slug);
  const mock = MOCK_POSTS.find((m) => m.slug === slug);
  const content = String(p.content ?? mock?.content ?? "");
  const category = (p.category ?? {}) as Record<string, unknown>;
  return {
    slug,
    title: String(p.title ?? mock?.title ?? slug),
    excerpt: String(p.excerpt ?? mock?.excerpt ?? ""),
    content,
    ...
    coverTheme: String(p.coverImage || mock?.coverTheme || "brand"),
    authorName: String(p.authorName ?? mock?.authorName ?? "Markala"),
    authorRole: String(p.authorRole ?? mock?.authorRole ?? ""),
    categorySlug: String(category.slug ?? p.categorySlug ?? mock?.categorySlug ?? ""),
    tags: Array.isArray(p.tags) ? (p.tags as string[]) : (mock?.tags ?? []),
    publishedAt: String(p.publishedAt ?? p.createdAt ?? mock?.publishedAt ?? new Date().toISOString()),
    readingMinutes: mock?.readingMinutes ?? estimateReadingMinutes(content),
    seoTitle: (p.seoTitle as string | null) ?? mock?.seoTitle ?? undefined,
    seoDescription: (p.seoDescription as string | null) ?? mock?.seoDescription ?? undefined,
  };
}

function mapCategory(c: Record<string, unknown>): BlogCategory {
  const slug = String(c.slug);
  const mock = MOCK_CATEGORIES.find((m) => m.slug === slug);
  return {
    slug,
    name: String(c.name ?? mock?.name ?? slug),
    description: String(c.description ?? mock?.description ?? ""),
  };
}
...
// ─────────────────────────────────────────────────────────────────────────────
// CANLI veri erişimcileri (server). Hata/boş → mock fallback.
// ─────────────────────────────────────────────────────────────────────────────

/** Tüm yayınlanmış yazılar (en yeni önce). API hatası/boş → mock fallback. */
export async function getBlogPosts(): Promise<BlogPost[]> {
  try {
    const data = await fetchJson("/blog/public/posts");
    if (!Array.isArray(data) || data.length === 0) return MOCK_POSTS;
...
/** Tek yazı (slug). API'de yoksa/hata → mock fallback. */
...
/** Tüm blog kategorileri. API hatası/boş → mock fallback. */
export async function getBlogCategories(): Promise<BlogCategory[]> {
  try {
    const data = await fetchJson("/blog/public/categories");
    if (!Array.isArray(data) || data.length === 0) return MOCK_CATEGORIES;
